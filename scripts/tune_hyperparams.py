...
ONLY 2022 and 2023 data — each split chronologically into a warm-start pool
(games before a per-season cutoff) and a validation slice (games from the
cutoff on), scored by POOLED walk-forward log loss across both seasons'
validation slices. 2024 stays completely untouched as the final holdout.

This is a second iteration of the tuning approach: a first pass tuned on
2023 alone and found the improvement barely transferred to the 2024
holdout (see docs/limitations.md §9) — a classic single-season overfitting
risk. Pooling two validation seasons is a real, if still partial,
mitigation (the original spec's nested time-series CV would use even more
seasons; this is a practical middle ground for the time budget).
...
CUTOFFS = {2022: pd.Timestamp("2022-07-24"), 2023: pd.Timestamp("2023-07-19")}
PARK_FACTOR_PRIOR = {2022: [2021], 2023: [2022]}
...
for season in TUNING_SEASONS:
    ...
    RAW[season] = dict(sched=sched, pg=pg, starters_raw=starters_raw, batters_raw=batters_raw,
                       lineups_raw=lineups_raw, pf=pf, weather=weather)
...
def build_gf(season: int, hp: dict) -> pd.DataFrame:
    ...
    lineup_off = build_lineup_offense_features(r["lineups_raw"], bproj, starter_hand)
...
# Includes the previous single-season "winners" (75/100 halflives) as
# explicit candidates, so this search can confirm or overturn them.
GRID = {
...
if __name__ == "__main__":
    ...
    print("FINAL TUNED HYPERPARAMETERS (multi-season):")
    print(best)
    final_score = score(best)
    print(f"Final pooled validation score: {final_score}")

    pd.DataFrame(results_log).to_csv(cfg.processed_dir / "tuning_log_v2_multiseason.csv", index=False)
    import json
    (cfg.processed_dir / "tuned_hyperparams_v2_multiseason.json").write_text(json.dumps(best, indent=2))
